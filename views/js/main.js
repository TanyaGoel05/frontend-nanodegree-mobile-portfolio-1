...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    // To avoid layout transhing
    var dx = determineDx(randomPizzaContainers[0], size);
    var newWidths = [];
    for (var i = 0; i < length; i++) {
      newwidths.push((randomPizzaContainers[i].offsetWidth + dx) + 'px');
    }
    for (var i = 0; i < length; i++) {
      randomPizzaContainers[i].style.width = newwidths[i];
    }
  }
...
//  var scrollTop = 1000;
  var itemTop = Math.sin(scrollTop / 1250);

  // To avoid layout thrashing
  var phases = [];
  var itemsLeft = [];
  for (var i = 0; i < length; i++) {
    phases.push(scrollTop+(i % 5));
    itemsLeft.push(items[i].basicLeft);
  }

  for (var i = 0; i< length; i++) {
    items[i].style.left = itemsLeft[i]+ 100 * phases[i] + 'px';
  }
